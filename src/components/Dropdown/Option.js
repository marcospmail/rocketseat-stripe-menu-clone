...
function DropdownOption({ name, content: Content, backgroundHeight }) {
  ...
  useEffect(() => {
    if (!registered && optionDimensions) {
      const WrappedContent = () => {
        ...
        useEffect(() => {
          const optionDimensions = contentRef.current.getBoundingClientRect()
          updateOptionProps(id, { optionDimensions })
        }, [])
